...
export const InsertCodeMap = [
  {
    ...
    title: i18n('Postman.Postman.292530-0')
  },
  {
    code: 'assert.equal(body.code, 0)',
    title: i18n('Postman.Postman.292530-1')
  },
  {
    code: 'assert.notEqual(status, 404)',
    title: i18n('Postman.Postman.292530-2')
  },
  {
    code: 'assert.notEqual(body.code, 40000)',
    title: i18n('Postman.Postman.292530-3')
  },
  {
    code: 'assert.deepEqual(body, {"code": 0})',
    title:  `${i18n('Postman.Postman.292530-4')} {"code": 0}`
  },
  {
    code: 'assert.notDeepEqual(body, {"code": 0})',
    title:  `${i18n('Postman.Postman.292530-5')} {"code": 0}`
  }
];
  ...
  const TooltipTitle = () => {
    return (
      <div>
        {example && (
          <div>
            {i18n('Postman.Postman.292530-6')} <span className="table-desc">{example}</span>
          </div>
        )}
        {desc && (
          <div>
            {i18n('Postman.Postman.292530-7')} <span className="table-desc">{desc}</span>
          </div>
        )}
      </div>
    );
...
export default class Run extends Component {
  ...
  reqRealInterface = async () => {
    ...
    if (!validResult.valid) {
      this.setState({ test_valid_msg: `${i18n('Postman.Postman.292530-8')}${validResult.message}` });
    } else {
    ...
    return (
      <div className="interface-test postman">
        ...
        {this.state.envModalVisible && (
          <Modal
            title={i18n('Postman.Postman.292530-9')}
        ...
        <div className="url">
          <InputGroup compact style={{ display: 'flex' }}>
            ...
            >
              ...
              <Option value={i18n('Postman.Postman.292530-10')} disabled style={{ cursor: 'pointer', color: '#2395f1' }}>
                <Button type="primary" onClick={this.showEnvModal}>
                  {i18n('Postman.Postman.292530-10')}
                </Button>
              </Option>
            </Select>
          ...
          <Tooltip
            ...
            title={(() => {
              if (hasPlugin) {
                return i18n('Postman.Postman.292530-11');
              } else {
                return i18n('Postman.Postman.292530-12');
              }
            })()}
          >
            <Button
            ...
            >
              {loading ? i18n('Postman.Postman.292530-13') : i18n('Postman.Postman.292530-14')}
            </Button>
          </Tooltip>
          ...
          <Tooltip
            placement="bottom"
            title={() => {
              return this.props.type === 'inter' ? i18n('Postman.Postman.292530-15') : i18n('Postman.Postman.292530-16');
            }}
          >
            <Button onClick={this.props.save} type="primary" style={{ marginLeft: 10 }}>
              {this.props.type === 'inter' ? i18n('Postman.Postman.292530-17') : i18n('Postman.Postman.292530-18')}
            </Button>
          </Tooltip>
        </div>
        ...
        <Collapse defaultActiveKey={['0', '1', '2', '3']} bordered={true}>
          ...
          >
              ...
              return (
                <div key={index} className="key-value-wrap">
                  ...
                  <Input
                    ...
                    placeholder={i18n('Postman.Postman.292530-19')}
            ...
            >
              {i18n('Postman.Postman.292530-20')}
            </Button>
          </Panel>
          <Panel
          ...
          >
            ...
            <Button style={{ display: 'none' }} type="primary" icon="plus" onClick={this.addQuery}>
              {i18n('Postman.Postman.292530-21')}
            </Button>
          </Panel>
          <Panel header="HEADERS" key="2" className={req_headers.length === 0 ? 'hidden' : ''}>
            ...
            <Button style={{ display: 'none' }} type="primary" icon="plus" onClick={this.addHeader}>
              {i18n('Postman.Postman.292530-22')}
            </Button>
          </Panel>
          <Panel
            header={
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <Tooltip title={i18n('Postman.Postman.292530-23')}>BODY(F9)</Tooltip>
              </div>
            }
            key="3"
            className={
            ...
            >
              {req_body_type === 'json' && (
                <div className="adv-button">
                  ...
                  >
                    {i18n('Postman.Postman.292530-24')}
                  </Button>
                  <Tooltip title={i18n('Postman.Postman.292530-25')}>
                    {'  '}
            ...
            {HTTP_METHOD[method].request_body &&
              req_body_type === 'form' && (
                <div>
                  {req_body_form.map((item, index) => {
                    return (
                      <div key={index} className="key-value-wrap">
                        ...
                        <span className="eq-symbol">=</span>
                        {item.type === 'file' ? (
                          i18n('Postman.Postman.292530-26')
                          // <Input
                          //   type="file"
                          //   id={'file_' + index}
                  ...
                  >
                    {i18n('Postman.Postman.292530-27')}
                  </Button>
                </div>
              )}
            {HTTP_METHOD[method].request_body &&
        ...
        <Tabs size="large" defaultActiveKey="res" className="response-tab">
          <Tabs.TabPane tab="Response" key="res">
            <Spin spinning={this.state.loading}>
              ...
              <div>
                <a rel="noopener noreferrer"  target="_blank" href="https://juejin.im/post/5c888a3e5188257dee0322af">{i18n('Postman.Postman.292530-33')}</a>
              </div>
              {this.state.test_valid_msg && (
                <Alert
                  message={
                    <span>
                      Warning &nbsp;
                      <Tooltip title={i18n('Postman.Postman.292530-28')}>
                        <Icon type="question-circle-o" />
                      </Tooltip>
                    </span>
                  }
                ...
                <div className="body">
                  <div className="container-title">
                    ...
                    <Checkbox
                      ...
                      <span>{i18n('Postman.Postman.292530-29')}</span>
                    </Checkbox>
                  </div>
                  {
          ...
          {this.props.type === 'case' ? (
            <Tabs.TabPane
              ...
              tab={<Tooltip title={i18n('Postman.Postman.292530-30')}>Test</Tooltip>}
              key="test"
            >
              <h3 style={{ margin: '5px' }}>
                &nbsp;{i18n('Postman.Postman.292530-31')}:&nbsp;
                <Switch
                  checked={this.state.enable_script}
                  onChange={e => this.setState({ enable_script: e })}
                />
              </h3>
              <p style={{ margin: '10px' }}>{i18n('Postman.Postman.292530-32')}</p>
              <Row>
                <Col span="18">
                  <AceEditor
