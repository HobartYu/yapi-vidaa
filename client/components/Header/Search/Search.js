    ...
    axios
      ...
      .then(res => {
        if (res.data && res.data.errcode === 0) {
                ...
                case 'group':
                  dataSource.push(
                    <Option
                      ...
                      id={`${item._id}`}
                    >
                      {`${i18n('Search.Search.915751-0')} ${item.groupName}`}
                    </Option>
                  );
                  break;
                case 'project':
                  dataSource.push(
                    ...
                    >
                      {`${i18n('Search.Search.915751-1')} ${item.name}`}
                    </Option>
                  );
                  break;
                case 'interface':
                  dataSource.push(
                    ...
                    >
                      {`${i18n('Search.Search.915751-2')} ${item.title}`}
                    </Option>
                  );
                  break;
                default:
          ...
          console.log(i18n('Search.Search.915751-3'));
        }
      })
      .catch(err => {
        console.log(err);
      ...
      <div className="search-wrapper">
        ...
        >
          <Input
            ...
            placeholder={i18n('Search.Search.915751-4')}
