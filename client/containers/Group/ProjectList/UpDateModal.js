...
class UpDateModal extends Component {
        ...
        updateProject(assignValue)
          .then(res => {
            if (res.payload.data.errcode == 0) {
              ...
              message.success(i18n('ProjectList.UpDateModal.905275-0'));
  ...
  render() {
      ...
      return (
        <Row key={index} type="flex" justify="space-between" align={index === 0 ? 'middle' : 'top'}>
          <Col span={10} offset={2}>
            <FormItem label={index === 0 ? <span>{i18n('ProjectList.UpDateModal.905275-1')}</span> : ''} required={false} key={index}>
                ...
                rules: [
                  {
                    ...
                    validator(rule, value, callback) {
                      if (value) {
                        if (value.length === 0) {
                          callback(i18n('ProjectList.UpDateModal.905275-2'));
                        } else if (!/\S/.test(value)) {
                          callback(i18n('ProjectList.UpDateModal.905275-2'));
                        } else if (/prd/.test(value)) {
                          callback(i18n('ProjectList.UpDateModal.905275-3'));
                        } else {
                          return callback();
                        }
                      } else {
              ...
              })(<Input placeholder={i18n('ProjectList.UpDateModal.905275-4')} style={{ width: '90%', marginRight: 8 }} />)}
            </FormItem>
          </Col>
          <Col span={10}>
            <FormItem
              label={index === 0 ? <span>{i18n('ProjectList.UpDateModal.905275-5')}</span> : ''}
              required={false}
              key={secondIndex}
            >
              {getFieldDecorator(`envs-domain-${index}`, {
                ...
                rules: [
                  {
                    required: false,
                    whitespace: true,
                    message: i18n('ProjectList.UpDateModal.905275-2'),
                    validator(rule, value, callback) {
                      if (value) {
                        if (value.length === 0) {
                          callback(i18n('ProjectList.UpDateModal.905275-2'));
                ...
                <Input
                  placeholder={i18n('ProjectList.UpDateModal.905275-2')}
    ...
    return (
      <Modal
        title={i18n('ProjectList.UpDateModal.905275-6')}
        visible={isUpdateModalShow}
        onOk={this.handleOk}
        onCancel={this.handleCancel}
      >
        <Form>
          <FormItem {...formItemLayout} label={i18n('ProjectList.UpDateModal.905275-7')}>
            {getFieldDecorator('name', {
              initialValue: initFormValues.name,
              rules: [
                {
                  required: true,
                  message: i18n('ProjectList.UpDateModal.905275-8')
                }
              ]
            })(<Input />)}
          </FormItem>
          ...
          <FormItem
            ...
            label={
              <span>
                {i18n('ProjectList.UpDateModal.905275-9')}&nbsp;
                <Tooltip title={i18n('ProjectList.UpDateModal.905275-10')}>
          ...
          >
            {getFieldDecorator('prd_host', {
              ...
              rules: [
                {
                  required: true,
                  message: i18n('ProjectList.UpDateModal.905275-11')
                }
              ]
            })(
              <Input
          ...
          <FormItem
            ...
            label={
              <span>
                {i18n('ProjectList.UpDateModal.905275-12')}&nbsp;
                <Tooltip title={i18n('ProjectList.UpDateModal.905275-13')}>
                  <Icon type="question-circle-o" />
                </Tooltip>
              </span>
            }
          >
            {getFieldDecorator('basepath', {
              ...
              rules: [
                {
                  required: false,
                  message: i18n('ProjectList.UpDateModal.905275-14')
                }
              ]
            })(<Input />)}
          </FormItem>

          <FormItem {...formItemLayout} label={i18n('ProjectList.UpDateModal.905275-15')}>
            {getFieldDecorator('desc', {
              initialValue: initFormValues.desc,
              rules: [
                {
                  required: false,
                  message: i18n('ProjectList.UpDateModal.905275-16')
                }
              ]
            })(<TextArea rows={4} />)}
          </FormItem>
          ...
          <FormItem {...formItemLayoutWithOutLabel}>
            <Button type="dashed" onClick={this.add} style={{ width: '60%' }}>
              <Icon type="plus" /> {i18n('ProjectList.UpDateModal.905275-17')}
            </Button>
          </FormItem>
        </Form>
      </Modal>
