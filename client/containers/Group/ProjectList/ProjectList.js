  ...
  render() {
    ...
    const Follow = () => {
      return followProject.length ? (
        <Row>
          <h3 className="owner-type">{i18n('ProjectList.ProjectList.037698-0')}</h3>
    ...
    const NoFollow = () => {
      return noFollow.length ? (
        <Row style={{ borderBottom: '1px solid #eee', marginBottom: '15px' }}>
          <h3 className="owner-type">{i18n('ProjectList.ProjectList.037698-1')}</h3>
      ...
      <div style={{ paddingTop: '24px' }} className="m-panel card-panel card-panel-s project-list">
        <Row className="project-list-header">
          <Col span={16} style={{ textAlign: 'left' }}>
            {i18n('ProjectList.ProjectList.037698-2', { groupName: this.props.currGroup.group_name, length: projectData.length })}
          </Col>
          <Col span={8}>
            {isShow ? (
              <Link to="/add-project">
                <Button type="primary">{i18n('ProjectList.ProjectList.037698-4')}</Button>
              </Link>
            ) : (
              <Tooltip title={i18n('ProjectList.ProjectList.037698-5')}>
                <Button type="primary" disabled>
                  {i18n('ProjectList.ProjectList.037698-4')}
                </Button>
              </Tooltip>
            )}
          </Col>
