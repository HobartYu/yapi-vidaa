...
class MemberList extends Component {
    ...
    this.props
      ...
      .then(res => {
        if (!res.payload.data.errcode) {
          ...
          message.success(i18n('MemberList.MemberList.170713-0', { addLength, existLength }) );
  ...
  render() {
    const columns = [
      {
        title: i18n('MemberList.MemberList.170713-1', { groupName: this.props.currGroup.group_name, length: this.state.userInfo.length}),
        dataIndex: 'username',
        key: 'username',
        render: (text, record) => {
          return (
          ...
          this.state.role === 'owner' || this.state.role === 'admin' ? (
            <div className="btn-container">
              <Button className="btn" type="primary" onClick={this.showAddMemberModal}>
                {i18n('MemberList.MemberList.170713-3')}
              </Button>
            </div>
          ) : (
            ''
          ...
          if (this.state.role === 'owner' || this.state.role === 'admin') {
            return (
              <div>
                ...
                >
                  <Option value={'owner-' + record.uid}>{i18n('MemberList.MemberList.170713-4')}</Option>
                  <Option value={'dev-' + record.uid}>{i18n('MemberList.MemberList.170713-5')}</Option>
                  <Option value={'guest-' + record.uid}>{i18n('MemberList.MemberList.170713-6')}</Option>
                </Select>
                <Popconfirm
                  placement="topRight"
                  title={i18n('MemberList.MemberList.170713-7')}
                  onConfirm={this.deleteConfirm(record.uid)}
                  okText={i18n('MemberList.MemberList.170713-8')}
                  cancelText=""
                >
            ...
            if (record.role === 'owner') {
              return i18n('MemberList.MemberList.170713-4');
            } else if (record.role === 'dev') {
              return i18n('MemberList.MemberList.170713-5');
            } else if (record.role === 'guest') {
              return i18n('MemberList.MemberList.170713-6');
            } else {
              return '';
            }
          }
    ...
    return (
      <div className="m-panel">
        {this.state.visible ? (
          <Modal
            title={i18n('MemberList.MemberList.170713-3')}
          ...
          >
            <Row gutter={6} className="modal-input">
              <Col span="5">
                <div className="label usernamelabel">{i18n('MemberList.MemberList.170713-9')}</div>
              </Col>
              <Col span="15">
            ...
            <Row gutter={6} className="modal-input">
              <Col span="5">
                <div className="label usernameauth">{i18n('MemberList.MemberList.170713-10')}</div>
              </Col>
              <Col span="15">
                <Select defaultValue="dev" className="select" onChange={this.changeNewMemberRole}>
                  <Option value="owner">{i18n('MemberList.MemberList.170713-4')}</Option>
                  <Option value="dev">{i18n('MemberList.MemberList.170713-5')}</Option>
                  <Option value="guest">{i18n('MemberList.MemberList.170713-6')}</Option>
                </Select>
              </Col>
            </Row>
          </Modal>
