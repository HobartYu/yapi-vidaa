  ...
  return (
    <Modal visible={visible} title={title} onCancel={onCancel} onOk={onCreate}>
      <Form layout="vertical">
        <FormItem label={i18n('InterfaceCol.InterfaceColMenu.870131-0')}>
          {getFieldDecorator('colName', {
            rules: [{ required: true, message: i18n('InterfaceCol.InterfaceColMenu.870131-1') }]
          })(<Input />)}
        </FormItem>
        <FormItem label={i18n('InterfaceCol.InterfaceColMenu.870131-2')}>{getFieldDecorator('colDesc')(<Input type="textarea" />)}</FormItem>
      </Form>
    </Modal>
  );
});
...
export default class InterfaceColMenu extends Component {
  ...
  addorEditCol = async () => {
    ...
    if (!res.data.errcode) {
      ...
      message.success(colModalType === 'edit' ? i18n('InterfaceCol.InterfaceColMenu.870131-3') : i18n('InterfaceCol.InterfaceColMenu.870131-4'));
  ...
  showDelColConfirm = colId => {
    ...
    confirm({
      title: i18n('InterfaceCol.InterfaceColMenu.870131-5'),
      content: i18n('InterfaceCol.InterfaceColMenu.870131-6'),
      okText: i18n('InterfaceCol.InterfaceColMenu.870131-7'),
      cancelText: i18n('InterfaceCol.InterfaceColMenu.870131-8'),
      async onOk() {
        const res = await axios.get('/api/col/del_col?col_id=' + colId);
        if (!res.data.errcode) {
          message.success(i18n('InterfaceCol.InterfaceColMenu.870131-9'));
  ...
  copyInterface = async item => {
    ...
    message.success(i18n('InterfaceCol.InterfaceColMenu.870131-10'));
  };

  showNoDelColConfirm = () => {
    confirm({
      title: i18n('InterfaceCol.InterfaceColMenu.870131-11'),
      content: i18n('InterfaceCol.InterfaceColMenu.870131-12')
    });
  };
  caseCopy = async caseId=> {
    ...
    const res = await axios.post('/api/col/add_case',data);
      if (!res.data.errcode) {
        message.success(i18n('InterfaceCol.InterfaceColMenu.870131-13'));
  ...
  showDelCaseConfirm = caseId => {
    ...
    const params = this.props.match.params;
    confirm({
      title: i18n('InterfaceCol.InterfaceColMenu.870131-14'),
      content: i18n('InterfaceCol.InterfaceColMenu.870131-15'),
      okText: i18n('InterfaceCol.InterfaceColMenu.870131-7'),
      cancelText: i18n('InterfaceCol.InterfaceColMenu.870131-8'),
      async onOk() {
        const res = await axios.get('/api/col/del_case?caseid=' + caseId);
        if (!res.data.errcode) {
          message.success(i18n('InterfaceCol.InterfaceColMenu.870131-16'));
          that.getList();
  ...
  handleImportOk = async () => {
    ...
    if (!res.data.errcode) {
      ...
      message.success(i18n('InterfaceCol.InterfaceColMenu.870131-17'));
      // await this.props.fetchInterfaceColList(project_id);
      this.getList();
        ...
        <TreeNode
          ...
          title={
            ...
            >
              ...
              <div className="btns">
                <Tooltip title={i18n('InterfaceCol.InterfaceColMenu.870131-18')}>
                  <Icon
                    type="delete"
                ...
                <Tooltip title={i18n('InterfaceCol.InterfaceColMenu.870131-19')}>
                  <Icon
                    type="copy"
                    className="interface-delete-icon"
                    onClick={e => {
    ...
    return (
      <div>
        <div className="interface-filter">
          <Input placeholder={i18n('InterfaceCol.InterfaceColMenu.870131-20')} onChange={this.filterCol} />
          <Tooltip placement="bottom" title={i18n('InterfaceCol.InterfaceColMenu.870131-21')}>
            <Button
              type="primary"
            ...
            >
              {i18n('InterfaceCol.InterfaceColMenu.870131-21')}
            </Button>
          </Tooltip>
        </div>
              ...
              <TreeNode
                ...
                title={
                  <div className="menu-title">
                    ...
                    <div className="btns">
                      <Tooltip title={i18n('InterfaceCol.InterfaceColMenu.870131-22')}>
                        <Icon
                          type="delete"
                      ...
                      <Tooltip title={i18n('InterfaceCol.InterfaceColMenu.870131-23')}>
                        <Icon
                          type="edit"
                          className="interface-delete-icon"
                          onClick={e => {
                      ...
                      <Tooltip title={i18n('InterfaceCol.InterfaceColMenu.870131-24')}>
                        <Icon
                          type="plus"
                          className="interface-delete-icon"
                          onClick={e => {
                      ...
                      <Tooltip title={i18n('InterfaceCol.InterfaceColMenu.870131-25')}>
                        <Icon
                          type="copy"
                          className="interface-delete-icon"
                          onClick={e => {
        ...
        <Modal
          title={i18n('InterfaceCol.InterfaceColMenu.870131-26')}
