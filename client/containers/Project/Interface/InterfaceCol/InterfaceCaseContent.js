...
export default class InterfaceCaseContent extends Component {
  ...
  updateCase = async () => {
    ...
    if (res.data.errcode) {
      ...
      message.success(i18n('InterfaceCol.InterfaceCaseContent.910222-0'));
    ...
    return (
      <div style={{ padding: '6px 0' }} className="case-content">
        <div className="case-title">
          {!isEditingCasename && (
            <Tooltip title={i18n('InterfaceCol.InterfaceCaseContent.910222-1')} placement="bottom">
          ...
          <span className="inter-link" style={{ margin: '0px 8px 0px 6px', fontSize: 12 }}>
            ...
            >
              {i18n('InterfaceCol.InterfaceCaseContent.910222-2')}
            </Link>
          </span>
        </div>
        <div>
          {Object.keys(currCase).length > 0 && (
            <Postman
              data={data}
              type="case"
              saveTip={i18n('InterfaceCol.InterfaceCaseContent.910222-3')}
