...
class InterfaceList extends Component {
  ...
  handleChangeInterfaceCat = (desc, name) => {
    ...
    axios.post('/api/interface/up_cat', params).then(async res => {
      ...
      message.success(i18n('InterfaceList.InterfaceList.975280-0'));
    });
  };
  ...
  handleAddInterface = data => {
    ...
    axios.post('/api/interface/add', data).then(res => {
      if (res.data.errcode !== 0) {
        return message.error(i18n('InterfaceList.InterfaceList.975280-1', { msg: res.data.errmsg }));
      }
      message.success(i18n('InterfaceList.InterfaceList.975280-2'));
  ...
  changeInterfaceCat = async (id, catid) => {
    ...
    if (result.data.errcode === 0) {
      message.success(i18n('InterfaceList.InterfaceList.975280-3'));
      this.handleRequest(this.props);
  ...
  render() {
    ...
    const columns = [
      {
        title: i18n('InterfaceList.InterfaceList.975280-4'),
        dataIndex: 'title',
        key: 'title',
        width: 30,
        render: (text, item) => {
      ...
      {
        title: i18n('InterfaceList.InterfaceList.975280-5'),
        dataIndex: 'path',
        key: 'path',
        width: 50,
        render: (item, record) => {
          ...
          return (
            <div>
              ...
              <Tooltip title={i18n('InterfaceList.InterfaceList.975280-6')} placement="topLeft">
      ...
      {
        title: i18n('InterfaceList.InterfaceList.975280-7'),
        dataIndex: 'catid',
        key: 'catid',
        width: 28,
        render: (item, record) => {
      ...
      {
        title: i18n('InterfaceList.InterfaceList.975280-8'),
        dataIndex: 'status',
        key: 'status',
        width: 24,
        render: (text, record) => {
          ...
          return (
            ...
            >
              <Option value={key + '-done'}>
                <span className="tag-status done">{i18n('InterfaceList.InterfaceList.975280-9')}</span>
              </Option>
              <Option value={key + '-undone'}>
                <span className="tag-status undone">{i18n('InterfaceList.InterfaceList.975280-10')}</span>
              </Option>
            </Select>
          );
        },
        filters: [
          {
            text: i18n('InterfaceList.InterfaceList.975280-9'),
            value: 'done'
          },
          {
            text: i18n('InterfaceList.InterfaceList.975280-10'),
            value: 'undone'
          }
        ],
      ...
      },
      {
        ...
        width: 14,
        render: text => {
          let textMsg = text.length > 0 ? text.join('\n') : i18n('InterfaceList.InterfaceList.975280-11');
    ...
    return (
      <div style={{ padding: '24px' }}>
        <h2 className="interface-title" style={{ display: 'inline-block', margin: 0 }}>
          {intername ? intername : i18n('InterfaceList.InterfaceList.975280-12', { total })}
        </h2>
        ...
        >
          {i18n('InterfaceList.InterfaceList.975280-15')}
        </Button>
        ...
        {this.state.visible && (
          <Modal
            title={i18n('InterfaceList.InterfaceList.975280-15')}
