...
export default class ProjectRequest extends Component {
  ...
  handleSubmit = async () => {
    ...
    if (result.payload.data.errcode === 0) {
      message.success(i18n('ProjectRequest.ProjectRequest.661195-0'));
      await this.props.getProject(this.props.projectId);
    } else {
      message.success(i18n('ProjectRequest.ProjectRequest.661195-1') + result.payload.data.errmsg);
    }
  };
        ...
        <Form onSubmit={this.handleSubmit}>
          ...
          <FormItem {...tailFormItemLayout}>
            <Button onClick={this.handleSubmit} type="primary">
              {i18n('ProjectRequest.ProjectRequest.661195-4')}
            </Button>
          </FormItem>
        </Form>
      </div>
