...
class ProjectEnv extends Component {
  ...
  addParams = (name, data) => {
    let newValue = {};
    data = { name: i18n('ProjectEnv.index.798126-0'), domain: '', header: [] };
    ...
    await this.props
      ...
      .then(res => {
        if (res.payload.data.errcode == 0) {
          ...
          message.success(i18n('ProjectEnv.index.798126-1'));
          if(this._isMounted) {
      ...
      .catch(() => {
        message.error(i18n('ProjectEnv.index.798126-2'));
      });
  }
  ...
  handleInputChange = (value, currentKey) => {
    ...
    newValue[currentKey].name = value || i18n('ProjectEnv.index.798126-0');
      ...
      return (
        ...
        >
          <span className="env-icon-style">
            <span className="env-name" style={{ color: item.name === i18n('ProjectEnv.index.798126-0') && '#2395f1' }}>
              {item.name}
            </span>
            <Popconfirm
              title={i18n('ProjectEnv.index.798126-3')}
              onConfirm={e => {
                e.stopPropagation();
                this.showConfirm(index, 'env');
              }}
              okText={i18n('ProjectEnv.index.798126-4')}
              cancelText={i18n('ProjectEnv.index.798126-5')}
            >
              <Icon
                type="delete"
            ...
            <div style={{ height: '100%', borderRight: 0 }}>
              <Row className="first-menu-item menu-item">
                <div className="env-icon-style">
                  <h3>
                    {i18n('ProjectEnv.index.798126-6')}&nbsp;<Tooltip placement="top" title={i18n('ProjectEnv.index.798126-7')}>
                      <Icon type="question-circle-o" />
                    </Tooltip>
                  </h3>
                  <Tooltip title={i18n('ProjectEnv.index.798126-8')}>
