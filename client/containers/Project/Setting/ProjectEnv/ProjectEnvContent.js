...
class ProjectEnvContent extends Component {
  ...
  render() {
        ...
        <Row gutter={2} key={index}>
            ...
            <FormItem>
              {getFieldDecorator('header[' + index + '].name', {
                ...
                <AutoComplete
                  ...
                  placeholder={i18n('ProjectEnv.ProjectEnvContent.017312-0')}
          ...
          <Col span={12}>
            <FormItem>
              ...
              })(<Input placeholder={i18n('ProjectEnv.ProjectEnvContent.017312-1')} style={{ width: '90%', marginRight: 8 }} />)}
            </FormItem>
          </Col>
          ...
          <Col span={10}>
            <FormItem>
              {getFieldDecorator(`${name}[${index}].name`, {
                ...
                <Input
                  placeholder={i18n('ProjectEnv.ProjectEnvContent.017312-2', { name })}
                  style={{ width: '200px' }}
    ...
    const envTpl = data => {
      return (
        <div>
          <h3 className="env-label">{i18n('ProjectEnv.ProjectEnvContent.017312-3')}</h3>
          <FormItem required={false}>
            {getFieldDecorator('env.name', {
              validateTrigger: ['onChange', 'onBlur'],
              initialValue: data.name === i18n('ProjectEnv.ProjectEnvContent.017312-4') ? '' : data.name || '',
              rules: [
                {
                  required: false,
                  whitespace: true,
                  validator(rule, value, callback) {
                    if (value) {
                      if (value.length === 0) {
                        callback(i18n('ProjectEnv.ProjectEnvContent.017312-5'));
              ...
              <Input
                ...
                placeholder={i18n('ProjectEnv.ProjectEnvContent.017312-5')}
                style={{ width: '90%', marginRight: 8 }}
              />
            )}
          </FormItem>
          <h3 className="env-label">{i18n('ProjectEnv.ProjectEnvContent.017312-6')}</h3>
          <FormItem required={false}>
            {getFieldDecorator('env.domain', {
              ...
              rules: [
                {
                  ...
                  validator(rule, value, callback) {
                    if (value) {
                      if (value.length === 0) {
                        callback(i18n('ProjectEnv.ProjectEnvContent.017312-7'));
                      } else if (/\s/.test(value)) {
                        callback(i18n('ProjectEnv.ProjectEnvContent.017312-8'));
                      } else {
                        return callback();
                      }
                    } else {
              ...
              <Input
                placeholder={i18n('ProjectEnv.ProjectEnvContent.017312-9')}
                style={{ width: '90%', marginRight: 8 }}
          ...
          <h3 className="env-label">
            ...
            <a
              ...
              style={{ marginLeft: 8 }}
            >
              <Tooltip title={i18n('ProjectEnv.ProjectEnvContent.017312-10')}>
    ...
    return (
      <div>
        ...
        <div className="btnwrap-changeproject">
          ...
          >
            {i18n('ProjectEnv.ProjectEnvContent.017312-11')}
          </Button>
        </div>
      </div>
    );
