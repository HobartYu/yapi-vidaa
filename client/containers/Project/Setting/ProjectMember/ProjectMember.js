...
class ProjectMember extends Component {
    ...
    this.props
      ...
      .then(res => {
        if (!res.payload.data.errcode) {
          ...
          message.success(i18n('ProjectMember.ProjectMember.538778-0', { addLength, existLength }));
  ...
  render() {
    ...
    const columns = [
      {
        title:
          i18n('ProjectMember.ProjectMember.538778-1', { project: this.props.projectMsg.name, number: this.state.projectMemberList.length}),
        dataIndex: 'username',
        key: 'username',
        render: (text, record) => {
          return (
            <div className="m-user">
              ...
              <Tooltip placement="top" title={i18n('ProjectMember.ProjectMember.538778-3')}>
                <span>
                  <Switch
                    size="small"
                    checkedChildren={i18n('ProjectMember.ProjectMember.538778-4')}
                    unCheckedChildren={i18n('ProjectMember.ProjectMember.538778-5')}
          ...
          this.state.role === 'owner' || this.state.role === 'admin' ? (
            <div className="btn-container">
              <Button className="btn" type="primary" icon="plus" onClick={this.showAddMemberModal}>
                {i18n('ProjectMember.ProjectMember.538778-6')}
              </Button>
              <Button className="btn" icon="plus" onClick={this.showImportMemberModal}>
                {i18n('ProjectMember.ProjectMember.538778-7')}
              </Button>
            </div>
          ) : (
            ''
          ...
          if (this.state.role === 'owner' || this.state.role === 'admin') {
            return (
              <div>
                ...
                >
                  <Option value={'owner-' + record.uid}>{i18n('ProjectMember.ProjectMember.538778-8')}</Option>
                  <Option value={'dev-' + record.uid}>{i18n('ProjectMember.ProjectMember.538778-9')}</Option>
                  <Option value={'guest-' + record.uid}>{i18n('ProjectMember.ProjectMember.538778-10')}</Option>
                </Select>
                <Popconfirm
                  placement="topRight"
                  title={i18n('ProjectMember.ProjectMember.538778-11')}
                  onConfirm={this.deleteConfirm(record.uid)}
                  okText={i18n('ProjectMember.ProjectMember.538778-12')}
                  cancelText=""
                >
            ...
            if (record.role === 'owner') {
              return i18n('ProjectMember.ProjectMember.538778-8');
            } else if (record.role === 'dev') {
              return i18n('ProjectMember.ProjectMember.538778-9');
            } else if (record.role === 'guest') {
              return i18n('ProjectMember.ProjectMember.538778-10');
            } else {
              return '';
            }
          }
    ...
    return (
      <div className="g-row">
        <div className="m-panel">
          {this.state.visible ? (
            <Modal
              title={i18n('ProjectMember.ProjectMember.538778-6')}
            ...
            >
              <Row gutter={6} className="modal-input">
                <Col span="5">
                  <div className="label usernamelabel">{i18n('ProjectMember.ProjectMember.538778-13')}</div>
                </Col>
                <Col span="15">
              ...
              <Row gutter={6} className="modal-input">
                <Col span="5">
                  <div className="label usernamelabel">{i18n('ProjectMember.ProjectMember.538778-14')}</div>
                </Col>
                <Col span="15">
                  <Select defaultValue="dev" className="select" onChange={this.changeNewMemberRole}>
                    <Option value="owner">{i18n('ProjectMember.ProjectMember.538778-8')}</Option>
                    <Option value="dev">{i18n('ProjectMember.ProjectMember.538778-9')}</Option>
                    <Option value="guest">{i18n('ProjectMember.ProjectMember.538778-10')}</Option>
                  </Select>
                </Col>
              </Row>
            </Modal>
          ...
          <Modal
            title={i18n('ProjectMember.ProjectMember.538778-7')}
          ...
          >
            <Row gutter={6} className="modal-input">
              <Col span="5">
                <div className="label usernamelabel">{i18n('ProjectMember.ProjectMember.538778-15')}</div>
              </Col>
              <Col span="15">
                <Select
                  showSearch
                  style={{ width: 200 }}
                  placeholder={i18n('ProjectMember.ProjectMember.538778-16')}
          ...
          <Card
            bordered={false}
            title={i18n('ProjectMember.ProjectMember.538778-17', { project: this.state.groupName, number: this.state.groupMemberList.length})}
            hoverable={true}
                  ...
                  <div key={index} className="card-item">
                    ...
                    <p className="item-name">
                      ...
                      {item.uid === this.props.uid ? (
                        <Badge
                          count={i18n('ProjectMember.ProjectMember.538778-18')}
                          style={{
                    ...
                    {item.role === 'owner' ? <p className="item-role">{i18n('ProjectMember.ProjectMember.538778-8')}</p> : null}
                    {item.role === 'dev' ? <p className="item-role">{i18n('ProjectMember.ProjectMember.538778-9')}</p> : null}
                    {item.role === 'guest' ? <p className="item-role">{i18n('ProjectMember.ProjectMember.538778-10')}</p> : null}
                  </div>
                );
              })
            ) : (
