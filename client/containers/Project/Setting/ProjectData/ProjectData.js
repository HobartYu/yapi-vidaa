...
class ProjectData extends Component {
  ...
  uploadChange = info => {
    ...
    if (status === 'done') {
      message.success(i18n('ProjectData.ProjectData.643914-0', { name: info.file.name }));
    } else if (status === 'error') {
      message.error(i18n('ProjectData.ProjectData.643914-1', { name: info.file.name }));
    }
  };
  ...
  handleFile = info => {
    if (!this.state.curImportType) {
      return message.error(i18n('ProjectData.ProjectData.643914-2'));
    }
    if (this.state.selectCatid) {
      ...
      message.error(i18n('ProjectData.ProjectData.643914-3'));
    }
  };
  ...
  showConfirm = async res => {
    ...
    const ref = confirm({
      title: i18n('ProjectData.ProjectData.643914-4'),
      width: 600,
      okType: 'danger',
      iconType: 'exclamation-circle',
      className: 'dataImport-confirm',
      okText: i18n('ProjectData.ProjectData.643914-5'),
      cancelText: i18n('ProjectData.ProjectData.643914-6'),
      content: (
        <div className="postman-dataImport-modal">
          ...
          <p className="info">{i18n('ProjectData.ProjectData.643914-7')}</p>
        </div>
      ),
      async onOk() {
  ...
  onUrlUpload = async () => {
    ...
    if (!this.state.swaggerUrl) {
      return message.error(i18n('ProjectData.ProjectData.643914-8'));
    }
    if (this.state.selectCatid) {
      this.setState({ showLoading: true });
      try {
      ...
      <div className="g-row">
        <div className="m-panel">
          <div className="postman-dataImport">
            <div className="dataImportCon">
              <div>
                <h3>
                  ...
                  >
                    <Tooltip title={i18n('ProjectData.ProjectData.643914-10')}>
              ...
              <div className="dataImportTile">
                <Select
                  placeholder={i18n('ProjectData.ProjectData.643914-2')}
              ...
              <div className="catidSelect">
                <Select
                  ...
                  showSearch
                  style={{ width: '100%' }}
                  placeholder={i18n('ProjectData.ProjectData.643914-11')}
              ...
              <div className="dataSync">
                <span className="label">
                  {i18n('ProjectData.ProjectData.643914-12')}&nbsp;
                  <Tooltip
                    title={
                      <div>
                        <h3 style={{ color: 'white' }}>{i18n('ProjectData.ProjectData.643914-13')}</h3>
                        <p>{i18n('ProjectData.ProjectData.643914-14')}</p>
                        <br />
                        <h3 style={{ color: 'white' }}>{i18n('ProjectData.ProjectData.643914-15')}</h3>
                        <p>
                          {i18n('ProjectData.ProjectData.643914-16')}
                        </p>
                        <br />
                        <h3 style={{ color: 'white' }}>{i18n('ProjectData.ProjectData.643914-18')}</h3>
                        <p>{i18n('ProjectData.ProjectData.643914-19')}</p>
                      </div>
                    }
                  >
                    <Icon type="question-circle-o" />
                ...
                <Select value={this.state.dataSync} onChange={this.onChange}>
                  <Option value="normal">{i18n('ProjectData.ProjectData.643914-13')}</Option>
                  <Option value="good">{i18n('ProjectData.ProjectData.643914-15')}</Option>
                  <Option value="merge">{i18n('ProjectData.ProjectData.643914-18')}</Option>
                </Select>
              ...
              {this.state.curImportType === 'swagger' && (
                <div className="dataSync">
                  <span className="label">
                    {i18n('ProjectData.ProjectData.643914-20')}&nbsp;
                    <Tooltip title={i18n('ProjectData.ProjectData.643914-21')}>
                      <Icon type="question-circle-o" />
                    </Tooltip>{' '}
                    &nbsp;&nbsp;
                  </span>
              ...
              {this.state.isSwaggerUrl ? (
                <div className="import-content url-import-content">
                  ...
                  >
                    {i18n('ProjectData.ProjectData.643914-22')}
                  </Button>
                </div>
              ) : (
                <div className="import-content">
                  <Spin spinning={this.state.showLoading} tip={i18n('ProjectData.ProjectData.643914-23')}>
                    <Dragger {...uploadMess}>
                      <p className="ant-upload-drag-icon">
                        <Icon type="inbox" />
                      </p>
                      <p className="ant-upload-text">{i18n('ProjectData.ProjectData.643914-24')}</p>
                      <p
            ...
            >
              <div>
                <h3>{i18n('ProjectData.ProjectData.643914-25')}</h3>
              </div>
              <div className="dataImportTile">
                <Select placeholder={i18n('ProjectData.ProjectData.643914-26')} onChange={this.handleExportType}>
              ...
              <div className="dataExport">
                <RadioGroup defaultValue="all" onChange={this.handleChange}>
                  <Radio value="all">{i18n('ProjectData.ProjectData.643914-27')}</Radio>
                  <Radio value="open">{i18n('ProjectData.ProjectData.643914-28')}</Radio>
                </RadioGroup>
              </div>
              <div className="export-content">
                {this.state.curExportType ? (
                  <div>
                    ...
                    <a
                      ...
                      <Button className="export-button" type="primary" size="large">
                        {' '}
                        {i18n('ProjectData.ProjectData.643914-29')}{' '}
                      </Button>
                    </a>
                    <Checkbox
                    ...
                    >
                      {i18n('ProjectData.ProjectData.643914-30')}&nbsp;
                      <Tooltip title={i18n('ProjectData.ProjectData.643914-31')}>
                        <Icon type="question-circle-o" />
                      </Tooltip>{' '}
                    </Checkbox>
                  </div>
                  ...
                  <Button disabled className="export-button" type="primary" size="large">
                    {' '}
                    {i18n('ProjectData.ProjectData.643914-31')}{' '}
                  </Button>
                )}
              </div>
            </div>
