...
class Profile extends Component {
  ...
  getUserInfo = id => {
    ...
    axios.get('/api/user/find?id=' + id).then(res => {
      ...
      if (curUid === +id) {
        ...
        this.props.setBreadcrumb([{ name: i18n('User.Profile.687130-1') + res.data.data.username }]);
      }
    });
  };
    ...
    axios.post('/api/user/update', params).then(
      res => {
        ...
        if (data.errcode === 0) {
          ...
          message.success(i18n('User.Profile.687130-2'));
        } else {
  ...
  updatePassword = () => {
    ...
    if (password != verify_pass) {
      return message.error(i18n('User.Profile.687130-3'));
    }
    let params = {
    ...
    axios.post('/api/user/change_password', params).then(
      res => {
        ...
        if (data.errcode === 0) {
          ...
          message.success(i18n('User.Profile.687130-4'));
  ...
  render() {
    ...
    let roles = { admin: i18n('User.Profile.687130-5'), member: i18n('User.Profile.687130-6') };
    let userType = '';
      ...
      userNameEditHtml = (
        <div>
          <Input
            ...
            placeholder={i18n('User.Profile.687130-7')}
          />
    ...
    if (this.state.roleEdit === false) {
      ...
      roleEditHtml = (
        <Select defaultValue={_userinfo.role} onChange={this.changeRole} style={{ width: 150 }}>
          <Option value="admin">{i18n('User.Profile.687130-5')}</Option>
          <Option value="member">{i18n('User.Profile.687130-6')}</Option>
        </Select>
      );
    }
      ...
      secureEditHtml = (
        <div>
          <Input
            ...
            placeholder={i18n('User.Profile.687130-10')}
            type="password"
            name="old_password"
            id="old_password"
          />
          <Input placeholder={i18n('User.Profile.687130-11')} type="password" name="password" id="password" />
          <Input placeholder={i18n('User.Profile.687130-12')} type="password" name="verify_pass" id="verify_pass" />
          <ButtonGroup className="edit-buttons">
            <Button
              className="edit-button"
              onClick={() => {
      ...
      <div className="user-profile">
        <div className="user-item-body">
          {userinfo.uid === this.props.curUid ? (
            <h3>{i18n('User.Profile.687130-13')}</h3>
          ) : (
            <h3>{userinfo.username} {i18n('User.Profile.687130-14')}</h3>
          )}

          <Row className="avatarCon" type="flex" justify="start">
            <Col span={24}>
              {userinfo.uid === this.props.curUid ? (
                <AvatarUpload uid={userinfo.uid}>{i18n('User.Profile.687130-15')}</AvatarUpload>
              ) : (
          ...
          <Row className="user-item" type="flex" justify="start">
            ...
            <Col span={4}>{i18n('User.Profile.687130-16')}</Col>
            <Col span={12}>{userinfo.uid}</Col>
          </Row>
          <Row className="user-item" type="flex" justify="start">
            <div className="maoboli" />
            <Col span={4}>{i18n('User.Profile.687130-7')}</Col>
          ...
          >
            <div className="maoboli" />
            <Col span={4}>{i18n('User.Profile.687130-17')}</Col>
          ...
          >
            <div className="maoboli" />
            <Col span={4}>{i18n('User.Profile.687130-18')}</Col>
            <Col span={12}>{userinfo.type === 'site' ? i18n('User.Profile.687130-19') : i18n('User.Profile.687130-20')}</Col>
          </Row>
          <Row className="user-item" type="flex" justify="start">
            <div className="maoboli" />
            <Col span={4}>{i18n('User.Profile.687130-21')}</Col>
            <Col span={12}>{formatTime(userinfo.add_time)}</Col>
          </Row>
          <Row className="user-item" type="flex" justify="start">
            <div className="maoboli" />
            <Col span={4}>{i18n('User.Profile.687130-22')}</Col>
          ...
          {userType ? (
            <Row className="user-item" type="flex" justify="start">
              <div className="maoboli" />
              <Col span={4}>{i18n('User.Profile.687130-23')}</Col>
  ...
  render() {
    ...
    return (
      <div className="avatar-box">
        <Tooltip
          placement="right"
          title={<div>{i18n('User.Profile.687130-24')}</div>}
        >
          <div>
            <Upload
...
function beforeUpload(file) {
  ...
  if (!isJPG && !isPNG) {
    message.error(i18n('User.Profile.687130-26'));
  }
  const isLt2M = file.size / 1024 / 1024 < 0.2;
  if (!isLt2M) {
    message.error(i18n('User.Profile.687130-27'));
  }
