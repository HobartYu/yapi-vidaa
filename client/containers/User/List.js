...
class List extends Component {
      ...
      .then(
        res => {
          if (res.data.errcode === 0) {
            message.success(i18n('User.List.531922-0'));
  ...
  async componentWillMount() {
    this.props.setBreadcrumb([{ name: i18n('User.List.531922-1') }]);
  }
  ...
  render() {
    ...
    let columns = [
      ...
      {
        title: i18n('User.List.531922-3'),
        dataIndex: 'role',
        key: 'role',
        width: 150
      },
      {
        title: i18n('User.List.531922-4'),
        dataIndex: 'up_time',
        key: 'up_time',
        width: 160
      },
      {
        title: i18n('User.List.531922-5'),
        key: 'action',
        width: '90px',
        render: item => {
          return (
            <span>
              ...
              <Popconfirm
                title={i18n('User.List.531922-6')}
                onConfirm={() => {
                  this.confirm(item._id);
                }}
                okText={i18n('User.List.531922-7')}
                cancelText={i18n('User.List.531922-8')}
              >
                <a style={{ display: 'block', textAlign: 'center' }} href="#">
                  {i18n('User.List.531922-9')}
                </a>
              </Popconfirm>
            </span>
          );
    ...
    return (
      <section className="user-table">
        <div className="user-search-wrapper">
          <h2 style={{ marginBottom: '10px' }}>{i18n('User.List.531922-10', { number: this.state.total })}</h2>
          <Search
            onChange={e => this.handleSearch(e.target.value)}
            onSearch={this.handleSearch}
            placeholder={i18n('User.List.531922-12')}
          />
        </div>
        <Table
          bordered={true}
