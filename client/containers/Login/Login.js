        ...
        if (this.props.isLDAP && this.state.loginType === 'ldap') {
          this.props.loginLdapActions(values).then(res => {
            if (res.payload.data.errcode == 0) {
              ...
              message.success(i18n('Login.Login.423150-0'));
            }
          });
        } else {
  ...
  render() {
    ...
    const emailRule =
      this.state.loginType === 'ldap'
        ...
        : {
            required: true,
            message: i18n('Login.Login.423150-1'),
            pattern: /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{1,})+$/
          };
    return (
      <Form onSubmit={this.handleSubmit}>
        ...
        {isLDAP && (
          <FormItem>
            <RadioGroup defaultValue="ldap" onChange={this.handleFormLayoutChange}>
              ...
              <Radio value="normal">{i18n('Login.Login.423150-2')}</Radio>
            </RadioGroup>
          </FormItem>
        )}
        {/* 用户名 (Email) */}
        ...
        <FormItem style={formItemStyle}>
          {getFieldDecorator('password', {
            rules: [{ required: true, message: i18n('Login.Login.423150-3') }]
          })(
            <Input
              style={changeHeight}
        ...
        <FormItem style={formItemStyle}>
          {getFieldDecorator('language', {
            rules: [{ required: true, message: i18n('Login.Login.423150-7') }]
          })(
