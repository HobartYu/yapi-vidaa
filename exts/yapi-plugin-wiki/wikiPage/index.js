...
class WikiPage extends Component {
  ...
  handleConflict = () => {
    ...
    s.onerror = () => {
      ...
      console.warn(i18n('wikiPage.index.193093-0'));
    };
  };
  ...
  handleWebsocketAccidentClose = (fn, callback) => {
    ...
    if (this.WebSocket) {
      ...
      if (this.WebSocket.readyState !== 1) {
        message.error(i18n('wikiPage.index.193093-1'));
      } else {
        fn();
      }
      callback(true);
  ...
  handleData = async params => {
    ...
    if (result.data.errcode === 0) {
      ...
      message.error(i18n('wikiPage.index.193093-2', { errmsg: result.data.errmsg }));
    }
  };
  ...
  onUpload = async (desc, markdown) => {
    ...
    if (result.data.errcode === 0) {
      ...
      message.error(i18n('wikiPage.index.193093-3', { errmsg: result.data.errmsg }));
    }
    this.endWebSocket();
    // this.WebSocket.send('end');
  };
          ...
          <div className="wiki-content">
            {isConflict && (
              <div className="wiki-conflict">
                ...
                <span>{i18n('wikiPage.index.193093-4')}</span>
              </div>
            )}
          </div>
          {!isEditor ? (
