  ...
  handleSubmit = e => {
    ...
    axios.post('/api/plugin/advmock/save', params).then(res => {
      if (res.data.errcode === 0) {
        message.success(i18n('yapi-plugin-advanced-mock.AdvMock.470228-0'));
      } else {
      ...
      <div style={{ padding: '20px 10px' }}>
        <div style={{ textAlign: 'center', marginBottom: 20 }}>
          <Radio.Group value={tab} size="large" onChange={this.handleTapChange}>
            <Radio.Button value="case">{i18n('yapi-plugin-advanced-mock.AdvMock.470228-1')}</Radio.Button>
            <Radio.Button value="script">{i18n('yapi-plugin-advanced-mock.AdvMock.470228-2')}</Radio.Button>
          </Radio.Group>
        </div>
        <div style={{ display: isShowCase ? 'none' : '' }}>
          <Form onSubmit={this.handleSubmit}>
            <FormItem
              label={
                <span>
                  {i18n('yapi-plugin-advanced-mock.AdvMock.470228-3')}&nbsp;<a
                    target="_blank"
                    rel="noopener noreferrer"
                    href={constants.docHref.adv_mock_script}
                  >
                    <Tooltip title={i18n('yapi-plugin-advanced-mock.AdvMock.470228-4')}>
            ...
            >
              <Switch
                ...
                checkedChildren={i18n('yapi-plugin-advanced-mock.AdvMock.470228-5')}
                unCheckedChildren={i18n('yapi-plugin-advanced-mock.AdvMock.470228-6')}
              />
            </FormItem>

            <FormItem label={i18n('yapi-plugin-advanced-mock.AdvMock.470228-7')} {...formItemLayout}>
              <div id="mock-script" style={{ minHeight: '500px' }} />
            </FormItem>
            <FormItem {...tailFormItemLayout}>
              <Button type="primary" htmlType="submit">
                {i18n('yapi-plugin-advanced-mock.AdvMock.470228-8')}
              </Button>
            </FormItem>
          </Form>
        </div>
