...
async function handle(
  ...
  const taskNotice = _.throttle((index, len)=>{
    messageSuccess(i18n('common.HandleImportData.622163-0', { index: index + 1, len }))
  }, 3000)
  ...
  const handleAddInterface = async info => {
    ...
    let existNum = 0;
    if (len === 0) {
      messageError(i18n('common.HandleImportData.622163-1'));
      callback({ showLoading: false });
      return;
    }
    ...
    for (let index = 0; index < res.length; index++) {
      ...
      if (dataSync !== 'normal') {
        ...
        if (result.data.errcode) {
          ...
          if (result.data.errcode == 40033) {
            callback({ showLoading: false });
            messageError(i18n('common.HandleImportData.622163-2'));
            break;
          }
        }
      }
      if (count === len) {
        callback({ showLoading: false });
        messageSuccess(i18n('common.HandleImportData.622163-3', { successNum, existNum }));
        return;
      }
